...
def compose_email(server, email, password):
    try:
        ...
        print("Do you want to add an audio attachment to the email? for 'yes' say y  and for 'no' say n ")
...
def read_seen_emails(server, email, password):
    try:
        ...
        if seen_count == 0:
            ...
            print("No seen emails in the inbox.")
            return
        elif seen_count == 1:
            speak("There is one seen email in the inbox.")
            print("There is one seen email in the inbox.")
            num_emails_to_read = 1
        else:
            speak("How many seen emails would you like to read?")
            print("How many seen emails would you like to read?")
            command = listen()
            num_emails_to_read = convert_spoken_number_to_int(command)
...
def read_email(server, email_address, password, email_id):
    try:
        ...
        if status == 'OK':
            ...
            while True:
                speak("Choose an action for this email to Reply the mail say reply or to Mark as Spam say spam or to Mark as starred say star or to Move to Trash say trash or say skip to Skip the actions. Say your choice: ")
                action = listen()
                print("You chosen"+ action)
...
def access_trash_folder(server):
    try:
        ...
        print("Accessing the trash folder")
        server.select("[Gmail]/Trash")
        choice = "trash"
        process_emails(server, choice)
    except Exception as e:
...
def access_starred_folder(server):
    try:
        # Access the starred folder
        print("Accessing starred folder.")
